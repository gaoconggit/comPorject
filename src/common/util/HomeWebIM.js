...
import {escape2Html, getStore} from "./ImUtils";
...
this.loginInfo = {
  ...
  'userSig': getStore('tim_sig'), //当前用户身份凭证，必须是字符串类型，选填
...
function webimLogin() {
  if (!webim.checkLogin()) {
    ...
    webim.login(loginInfo, listeners, options, (resp) => {
        ...
        _applyJoinBigGroup(avChatRoomId); //加入大群
      }, (err) => {
        alert(err.ErrorInfo);
      }
    );
  } else {
    _applyJoinBigGroup(this.avChatRoomId); //加入大群
  }
}

//加群
function _applyJoinBigGroup(sekToID) {
  let options = {
    GroupId: sekToID,
  };
